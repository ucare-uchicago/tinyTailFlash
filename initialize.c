...
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	strncpy(ssd->outputfilename,"out.dat",7);
	strncpy(ssd->statisticfilename,"statistic.dat",13);
	...
	ssd->gc_per_group=fopen("data/raw-data/gc_per_group","w");
	if(ssd->statisticfile==NULL)
		{
			printf("the gc_per_group can't open\n");
			return NULL;
		}	

	ssd->gc_per_req=fopen("data/raw-data/gc_per_req","w");
	if(ssd->statisticfile==NULL)
		{
			printf("the gc_per_req can't open\n");
			return NULL;
		}
	
	ssd->process_time=fopen("data/raw-data/process_time","w");
	if(ssd->statisticfile==NULL)
		{
			printf("the proces_time can't open\n");
			return NULL;
		}
       
	ssd->gc_ratio=fopen("data/raw-data/gc_ratio","w");
	if(ssd->gc_ratio==NULL)
		{
			printf("the gc_ratio can't open: %s\n",strerror(errno));
			return NULL;
		}       
	
	ssd->request_size=fopen("data/raw-data/request_size","w");
	if(ssd->request_size==NULL)
		{
			printf("the request_size can't open\n");
			return NULL;
		}

	ssd->read_time=fopen("data/raw-data/read_time","w");
	if(ssd->read_time==NULL)
		{
			printf("the read_time can't open\n");
			return NULL;
		}

	ssd->write_time=fopen("data/raw-data/write_time","w");
	if(ssd->write_time==NULL)
		{
	      printf("the write_time can't open\n");
	      return NULL;
		}
...
struct dram_info * initialize_dram(struct ssd_info * ssd)
{
	...
	dram->map->parity_entry = (struct entry *)malloc(sizeof(struct entry) * page_num); //每个物理页和逻辑页都有对应关系
	alloc_assert(dram->map->parity_entry,"dram->map->parity_entry");
	memset(dram->map->parity_entry,0,sizeof(struct entry) * page_num);

	dram->rotating_gc = 1;
	int i, j;
	for (i = 0; i < ssd->parameter->channel_number; i++)
		for (j = 0; j < ssd->parameter->chip_channel[i]; j++) 
			ssd->dram->gc_monitor[i][j].write_interval = MAX_INT64;		
...
struct parameter_value *load_parameters(char parameter_file[30])
{
	...
	while(fgets(buf,200,fp)){
		...
		if((res_eql=strcmp(buf,"chip number")) ==0){			
			...
			}else if((res_eql=strcmp(buf,"rotating gc")) ==0){
			sscanf(buf + next_eql,"%d",&p->rotating_gc); 		  
		}else if((res_eql=strcmp(buf,"dynamic rotating gc")) ==0){
			sscanf(buf + next_eql,"%d",&p->dynamic_rotating_gc); 		  
		}else if((res_eql=strcmp(buf,"proactive read")) ==0){
			sscanf(buf + next_eql,"%d",&p->proactive_read); 		  
		}else if((res_eql=strcmp(buf,"nonblocking gc")) ==0){
			sscanf(buf + next_eql,"%d",&p->nonblocking_gc); 		  	      
		}else if((res_eql=strcmp(buf,"adaptive read")) ==0){
			sscanf(buf + next_eql,"%d",&p->adaptive_read); 		  	      
		}else if((res_eql=strcmp(buf,"tt pread")) ==0){
			sscanf(buf + next_eql,"%d",&p->tt_pread);		  
		}else if((res_eql=strcmp(buf,"raid")) ==0){
			sscanf(buf + next_eql,"%d",&p->raid);		  
		}else if((res_eql=strcmp(buf,"buffer background flush")) ==0){
			sscanf(buf + next_eql,"%d",&p->buffer_background_flush);		  
		}else{
