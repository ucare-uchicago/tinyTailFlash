...
#define CHIP_COPYBACK_SUSPEND 104 
#define CHIP_R_DATA_TRANSFER_SUSPEND 105
#define CHIP_W_DATA_TRANSFER_SUSPEND 106
...
#define SR_R_DATA_TRANSFER_SUSPEND 207
#define SR_W_DATA_TRANSFER_SUSPEND 208
...
struct ssd_info{
   //all values have been memset to 0, then initialized respectively 
   int reqs_with_gc;                  
   int reqs_without_gc;
   int groups_with_gc;
   int groups_without_gc;
   double avg_gc_count;
   int gc_added;
   int gc_completed;  
   int uninterrupt_gc_deleted;
   int interrupt_gc_deleted;
   int gc_background;
   int gc_foreground;
   FILE *gc_per_group;
   FILE *gc_per_req;
   ...
   FILE *read_time;
   FILE *write_time;
   int64_t start_time;                  //first request arrival time (to calculate length of workload)
   unsigned int delayed_requests;       //# of requests with queuing delay
   int merge_read;                      //# of merge read
   int64_t stats_time;                  //time to start to collect stats
   int gc_queue_length;
...
struct channel_info{
   int gc_chip;                         //indicate which chip has incomplete gc on the channel (for nonblocking gc)
...
struct request{
   ...
   unsigned int *gc_count; //record the # of gc encountered for a read request                     
   unsigned int stripe_count; //# of stripes in this request
   unsigned int *sub_req_dropped; //# of sub request dropped  
...
struct sub_request{
   ...
   int parity; //indicate whether this sub_request is parity write or not.
   struct sub_request *parity_read; //parity read sub_requests (to calculate parity)
};
...
struct parameter_value{
   ...
   int buffer_management;          //indicates that there is buffer management or not
   int scheduling_algorithm;       //记录使用哪种调度算法，1:FCFS
   float quick_radio;
   int related_mapping;

   unsigned int time_step;
   unsigned int small_large_write; //the threshold of large write, large write do not occupt buffer, which is written back to flash directly
   ...
   int nonblocking_gc;
   int adaptive_read;
   int tt_pread; //whether calculate cost and convert random read to sequential read
   int raid;
   int buffer_background_flush;
};
...
struct gc_operation{          
   ...
   unsigned int die;
   unsigned int plane;
   unsigned int block;           //该参数只在可中断的gc函数中使用（gc_interrupt），用来记录已经找出来的目标块号
   unsigned int page;            //该参数只在可中断的gc函数中使用（gc_interrupt），用来记录已经完成的数据迁移的页号

   unsigned int phase;           // to record phase for nonblocking_uninterrupt_gc
